...
/**
 * One Device, One connection, One {@link LiteBluetooth} Instance. One connection can has many
 * callback.
 * <p/>
...
public class LiteBluetooth {
    ...
    /**
     * Note: Be Sure Call This On Main(UI) Thread! Note: Be Sure Call This On Main(UI) Thread! Note:
     * Be Sure Call This On Main(UI) Thread!
     * <p/>
    ...
    public synchronized BluetoothGatt connect(final BluetoothDevice device,
            final boolean autoConnect, final LiteBleGattCallback callback) {
        Log.i(TAG, "connect device：" + device.getName() + " mac:" + device.getAddress()
     ...
     * GATT client. The callback is used to deliver results to Caller, such as connection status as
     * well as any further GATT client operations.
     * 
     ...
     * @param callback GATT callback handler that will receive asynchronous callbacks.
     */
    public boolean scanAndConnect(String mac, final boolean autoConnect,
    ...
    /**
     * Clears the device cache. After uploading new hello4 the DFU target will have other services
     * than before.
     */
    public boolean refreshDeviceCache() {
        /*
         * There is a refresh() method in BluetoothGatt class but for now it's hidden. We will call
         * it using reflections.
         */
        try {
    ...
    /**
     * return {@link #STATE_DISCONNECTED} {@link #STATE_SCANNING} {@link #STATE_CONNECTING}
     * {@link #STATE_CONNECTED} {@link #STATE_SERVICES_DISCOVERED}
     */
    ...
    private LiteBleGattCallback coreGattCallback = new LiteBleGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            if (BleLog.isPrint) {
                BleLog.i(TAG, "onConnectionStateChange  status: " + status + " ,newState: "
                        + newState + "  ,thread: " + Thread.currentThread().getId());
            }
